module.exports = (app) => {
  ...
  return {
    ...
    findPosts: async (req, res, done) => {
      try {
        const skipByValue = parseInt(req.query.skipByValue, 10);
      ...
      try {
        if (req.file) {
          ...
          const unlinkError = await fs.unlink(
            `./${req.body.oldImage}`,
            err => (err ? 'There was a problem updating the old post image!' : false),
          );
    ...
    deletePost: async (req, res, done) => {
      try {
        ...
        const unlinkError = await fs.unlink(
          `./${path}`,
          err => (err ? 'Unable to remove the image from the uploads folder!' : false),
        );
